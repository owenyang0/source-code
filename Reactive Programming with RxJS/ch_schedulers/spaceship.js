...
function isVisible(obj) {
  return obj.y > -40 && obj.y < canvas.height + 40 &&
...
function timedScheduler(t) {
  ...
  function scheduleAbsolute(state, dueTime, action) {
    return this.scheduleWithRelativeAndState(
      state, (dueTime + t) - this.now(), action);
  }

  return new Rx.Scheduler(
    Date.now, scheduleNow, scheduleRelative, scheduleAbsolute);
}
...
function generateEnemyShots(enemy, scheduler) {
  return Rx.Observable.generate(
    ...
    function() { return enemy; }, // Condition to keep generating
    function() {
      return { // Generate shot at the current enemy position
        x: enemy.x,
        y: enemy.y
      };
    },
    function(shot) { return shot; }, // Simply return the shot we just created
...
var Enemies = Rx.Observable.interval(ENEMY_FREQ)
  .scan(function(enemyArray) {
    ...
    var enemyShots = generateEnemyShots(enemy, timedScheduler)
...
var Game = Rx.Observable
  ...
  .sample(SPEED, Rx.Scheduler.requestAnimationFrame)
